...
通过将聚类中心经纬度代入高德地图拾取坐标系统，并结合反向地理编码API交叉验证，可识别出各聚类对应的真实地理区域，如表\ref{tab:cluster_centers}所示。
...
3 & 121.4778 & 31.2705 & \textbf{虹口区\footnote{聚类3中心(121.4778$^{\circ}$E, 31.2705$^{\circ}$N)实际位于虹口区鲁迅公园附近，但其K-Means聚类覆盖范围向东延伸至静安区、人民广场市政府区域及陆家嘴金融区，覆盖了上海核心CBD地带的早晚通勤起终点，可统称为CBD核心区。}（早高峰净流入站）} \\
...
10 & 121.4105 & 31.2594 & \textbf{普陀区（早高峰净流出站）} \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table}
...
识别结果：
\begin{itemize}
    \item \textbf{早高峰净流入最大站（CBD）}：Cluster 3 $(121.4778{}^{\circ}$E, $31.2705{}^{\circ}$N)，对应虹口区，其K-Means聚类覆盖范围包含静安区、人民广场市政府及陆家嘴金融区等CBD核心通勤地带。早高峰净流入$+152$——大量通勤者骑行至此换乘地铁或到达办公地点。
    \item \textbf{早高峰净流出最大站（RES）}：Cluster 10 $(121.4105{}^{\circ}$E, $31.2594{}^{\circ}$N)，对应上海市普陀区石泉路街道，临近中山公园，覆盖大量密集住宅区。早高峰净流出$-215$——大量居民从此处借车出发。
\end{itemize}
...
\caption{双站点空站与满站风险概率（24小时时序）：CBD空站风险与满站风险形成两个明显错峰，RES两类风险分布更缓和一致}
\label{fig:dual_risk}
\end{figure}
...
\textbf{(1) CBD站调度成本反增9.0\%的本质原因——过度控制（Over-controlling）。}
在潮汐微弱或容量受限时，单次物理调度的巨额沉没成本会导致边际收益倒挂，进而导致调度策略的经济性破产。CBD站调度后成本由41.20元微增至44.92元（+3.72元），该增幅可精确分解：全天仅触发\textbf{1次}调出（固定成本$C_{\text{dispatch}}=50$元），挽回约46.28元满站惩罚，净亏损3.72元。这并非"高频死循环"，而是\textbf{过度控制}的典型案例：早高峰(10:00)$P(N=K)=5.28\%$的满站风险确实存在，但期望损失（41.20元/天）本就不大——算法在检测到风险后"主动出击"，50元干预成本略超46.28元惩罚节省，净亏3.72元。症结在于\textbf{过度敏感（Over-sensitivity）}：一次边际成本略高于边际收益的干预被批准，而算法本可"Do Nothing"。这给出了清晰的边界判据——当$\mathbb{E}[\text{惩罚节省}] \lesssim C_{\text{dispatch}}$时，最优策略是\textbf{关闭该站点调度通道}，将运力集中到边际收益为正的站点。

\textbf{(2) RES站调度节省41.0\%的成功逻辑。}
相比之下，RES站$K=80$的充裕容量使满站风险极低（2.43\%），调度的焦点集中于空站风险（2.64\%）。RES站的调度之所以有效，并非因为其触发频率低，而是因为\textbf{单次调度的边际收益覆盖了边际成本}：每次调入操作以50元成本阻止约2--4元的空站损失，虽仍有净亏损，但由于净流出后的自然恢复周期更长（需要晚高峰还车流才能补充），一次调入的效果可持续数小时——相当于单次50元投入"购买"了数小时内多次免于空站损失的保护，累计期望节省在较长时间窗口内逼近甚至超过50元门槛。而在CBD站，同样的50元投入仅能"购买"10--15分钟的保护，累计期望节省远不足以覆盖成本。\textbf{两站对比揭示的不是"高频 vs 低频"的行为差异，而是同一成本结构在不同流入/流出动态下的盈亏分界}：边际收益是否倒挂，取决于$\lambda(t),\mu(t)$的相对强度以及由此决定的"调度效果半衰期"。RES站的调度是\textbf{正边际收益区间内的有效干预}：一次到位、效果持久、循环可控。
...
\caption{动态调度阈值与小时成本率：CBD站约1次调出净亏3.72元，属典型的过度控制（Over-controlling）——算法在本可"Do Nothing"时选择了干预}
...
\begin{enumerate}
    \item 通过K-Means空间聚类，成功从102,361条上海摩拜单车订单数据中自动识别出一对互补的潮汐站点：净流入站（Cluster 3，虹口区/CBD核心区）和净流出站（Cluster 10，普陀区/大型住宅区），并量化了两个站点24小时净流量近乎镜像对称的潮汐特征。
